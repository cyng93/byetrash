...
ENV INSTALL_PATH /dockerized_trashterminator
# The name of the application is dockerized_trashterminator and while there
...
# having to CD into the /dockerized_trashterminator folder every time would be
# lame, so instead we can set the WORKDIR to be /dockerized_trashterminator.
#
...
# dockerized_trashterminator/ path inside of the Docker image.
#
# It copies it to /dockerized_trashterminator because of the WORKDIR being set.
#
...
# /dockerized_trashterminator folder inside of the Docker image.
#
